...
public class TestBall {
    ...
    public TestBall(String heightFun, TheCourse course) {
        this.mass = course.getMass();//in kilograms  //default
        this.g = course.getGravity();
        this.coefficient_of_friction = course.getFriction();
        //this.function = f;
        this.gravitational_force = new Force(0, 0);
        this.friction_force = new Force(0, 0);
        function = new HeightSolver(heightFun);

        Vector2D position2D = new Vector2D(0, 0);
        this.position = new Vector3D(0, 0, function.evaluate(position2D));

        Vector2D oldPosition2D = new Vector2D(0, 0);
        this.old_position = new Vector3D(0, 0, function.evaluate(oldPosition2D));
    ...
    private void calculate_previous_position_once()
    {
        ...
        Vector2D position2D = new Vector2D(x_x,x_y);
        double x_z = this.function.evaluate(position2D);
    ...
    private void update_position()
    {
        ...
        Vector2D position2D = new Vector2D(p_x,p_y);
        double p_z = this.function.evaluate(position2D);
    ...
    public void setPosition(double x, double y)
    {
        ...
        Vector2D setPosition2D = new Vector2D(x,y);
        this.position.set_z(function.evaluate(setPosition2D));
    }
